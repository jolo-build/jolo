...
Structured Codex, Claude Code, and ACP sessions receive the shared `jolo` stdio MCP server; its `search_text` tool is pinned to the task's workspace and checked by the engine's inspection grant. Other MCP configuration stays in effect. The vendor agent decides which search tool to call; Jolo does not replace its own shell or native grep tools. Terminal-only agents are unchanged.

Validation: `bun test tests/integration/tgrep.test.js apps/engine/test/hosted-mcp.test.js`. Native integration cases skip when tgrep is absent; run setup first to exercise the real index. `tests/integration/packaging.test.js` validates the release layout.
