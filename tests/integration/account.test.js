...
test('CLI task approval shares identity with desktop; native and hosted agents receive immutable task revisions', async () => {
  ...
  try {
    ...
    const teamResponse=await post('/teams',{name:'Independent tickets',prefix:'JOLO'});
    ...
    expect((await desktop.call('task.get',{key:'jolo-1'})).task.team.id).toBe(team);
    const linked=(await desktop.call('run.start',{sessionId:session.id,requestId:'scoped-team',prompt:'@codex fix #JOLO-1'})).run;
