...
test('CLI task approval shares identity with desktop; native and hosted agents receive immutable task revisions', async () => {
  ...
  try {
    ...
    const teamResponse=await post('/teams',{name:'Independent tickets',prefix:'CYPHO'});
    ...
    expect(teamPath).toEndWith('/CYPHO-1');
    expect((await desktop.call('task.get',{key:'CYPHO-1',team})).task.description).toBe('TEAM_SCOPED_CONTEXT');
    expect((await desktop.call('task.get',{key:'cypho-1'})).task.team.id).toBe(team);
    const linked=(await desktop.call('run.start',{sessionId:session.id,requestId:'scoped-team',prompt:'@codex fix #CYPHO-1'})).run;
