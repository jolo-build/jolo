...
export function TaskRail({ call, revision, sessionId, onOpen }) {
  ...
  return <nav className="task-rail" aria-label="Switch tasks and chats">
    <div className="task-rail-marks">{rows.map(task => {
      ...
      const reveal = event => { const rect = event.currentTarget.getBoundingClientRect(); setHover({ id: task.sessionId, title, workspace: task.standalone ? 'Chat' : task.projectName, status, left: rect.left, top: rect.top }); };
    ...
    {hover && createPortal(<div className="task-rail-tooltip" role="tooltip" style={{ right: `${Math.max(8, window.innerWidth - hover.left + 10)}px`, top: `${Math.max(8, Math.min(hover.top - 8, window.innerHeight - 82))}px` }}><strong>{hover.title}</strong><span>{hover.workspace} · {hover.status}</span></div>, document.body)}
  </nav>;
}
