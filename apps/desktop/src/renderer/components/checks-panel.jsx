...
import { verificationLabel } from '../presentation.js';
export function ChecksPanel({ verification }) {
  const checks = verification?.checks ?? [];
  return <div className="checks-panel">
    <div className="checks-heading"><strong>Checks</strong><span className={verification?.status === 'passed' ? 'good' : 'muted'}>{verificationLabel(verification)}</span></div>
