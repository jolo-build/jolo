...
// Packaged layout: main.mjs sits at the app root next to dist/, preload/, and engine/. Development: src/main/.
...
const STALENESS_ROOTS = [path.join(REPO_ROOT, "apps", "engine", "src"), path.join(REPO_ROOT, "packages"), path.join(REPO_ROOT, "migrations")];
  ...
  ipcMain.handle("jolo:answererMenu", (event, options) => {
    ...
    return new Promise((resolve) => {
      ...
      Menu.buildFromTemplate([
        ...
        { type: "separator" },
        { label: "Provider settings…", click: () => { choice = "settings"; } },
...
app.whenReady().then(async () => {
  ...
  const loadingSmoke = SMOKE && process.env.JOLO_LOADING_SMOKE === '1'
    ? (await import('./loading-smoke.mjs')).prepareLoadingSmoke({ window, bridge, root: ROOT }) : null;
  ...
  if (SMOKE) {
    try {
      if (loadingSmoke) await loadingSmoke();
        ...
        const { runSmoke } = await import(new URL("./smoke.mjs", import.meta.url).href);
