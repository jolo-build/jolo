...
import { runBrowserChatSmoke, runBrowserContentionSmoke } from './browser-chat-smoke.js';
...
export async function runSmoke(window, bridge, browserHost, { ROOT, BUILD, log }) {
  ...
  if (process.env.JOLO_DELEGATION_SMOKE === '1') {
    const { runDelegationSmoke } = await import('./delegation-smoke.js');
    try {
      await runDelegationSmoke({ window, bridge, evaluate, waitFor, report, results });
      writeFileSync(path.join(results, 'smoke.json'), JSON.stringify(report, null, 2));
    } finally { fixture.close(); }
    return;
  }
  ...
  if (process.env.JOLO_CHANGE_SUMMARY_SMOKE === '1') {
    ...
    try {
      await runChangeSummarySmoke({ window, project, evaluate, waitFor, report, results });
      writeFileSync(path.join(results, 'smoke.json'), JSON.stringify(report, null, 2));
    } finally { fixture.close(); }
    return;
  }
  ...
  if (process.env.JOLO_VISUALIZATION_SMOKE === '1') {
    const { runVisualizationSmoke } = await import('./visualization-smoke.js');
    try {
      await runVisualizationSmoke({ window, bridge, project, results, evaluate, waitFor, report });
      writeFileSync(path.join(results, 'smoke.json'), JSON.stringify(report, null, 2));
    } finally { fixture.close(); }
    return;
  }
  if (process.env.JOLO_BOARD_SMOKE === '1') {
    const { runBoardSmoke } = await import('./board-smoke.js');
    try {
  ...
  if (process.env.JOLO_HISTORY_SMOKE === '1') {
    const { runHistorySmoke } = await import('./history-smoke.js');
    try {
  ...
  if (process.env.JOLO_TASKS_SMOKE === '1') {
    const { runTasksSmoke } = await import('./tasks-smoke.js');
    try {
  ...
  if (process.env.JOLO_ATTACHMENTS_SMOKE === '1') {
    const { runAttachmentsSmoke } = await import('./attachments-smoke.js');
    try {
  ...
  if (process.env.JOLO_LIVE_RESULTS_SMOKE === "1") {
    const { runLiveResultsSmoke } = await import('./live-results-smoke.js');
  ...
  }
  {
    ...
    await evaluate("window.__joloSmoke.showTask()");
    const { runQueueSmoke } = await import('./queue-smoke.js');
