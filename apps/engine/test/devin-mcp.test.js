...
test('Devin gets isolated run credentials while its existing settings and MCP servers are preserved', () => {
  ...
  const mcp = '{"mcpServers":{"existing":{"command":"existing"},"jolo_browser":{"command":"stale"},"jolo_custom":{"command":"theirs"}}}';
  ...
  const servers = [{ name: 'jolo', command: 'bun', args: ['jolo-mcp', '--token-file', '/run/one'], env: [] }];
  let first, second;
  try {
    first = prepareDevinMcp(env, servers, { platform: 'darwin', temporaryRoot: root });
    second = prepareDevinMcp(env, [{ ...servers[0], args: ['jolo-mcp', '--token-file', '/run/two'] }], { platform: 'darwin', temporaryRoot: root });
    ...
    // The stale jolo_browser injection from an older version is gone; the run's jolo server replaces it.
    // A user's own jolo_* server is not Jolo-managed and survives the strip.
    expect(read(first).mcpServers).toEqual({ legacy: { command: 'legacy' }, existing: { command: 'existing' }, jolo_custom: { command: 'theirs' }, jolo: { command: 'bun', args: servers[0].args, env: {} } });
    expect(read(second).mcpServers.jolo.args.at(-1)).toBe('/run/two');
