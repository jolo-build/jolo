...
export function inlineBrowserInstructions({ available, hosted = false }) {
  return [
    ...
    available
      ? `${hosted ? 'The jolo MCP server provides the browser tools for this run. Discover its tools if your client defers tool loading. ' : ''}Call browser_open to open or reveal the pane, await it, then browser_navigate to the requested URL and browser_snapshot to inspect the page. Use browser_click, browser_fill, browser_press, and the other browser tools with snapshot references to interact. You can open the pane yourself; do not ask the user to click Browser. An empty browser_tabs result means no tab is attached yet; call browser_open. Browser connections can change during a run; check with the tools instead of assuming a connection is missing. If the desktop reconnects after a browser_host_unavailable error, retry browser_open.`
      : 'The dedicated browser tools are not available in this engine. If browser control is requested, explain that limitation; do not try a different automation tool.',
