...
export const canCommentTask = (task, actor) => !task.archived_at && (!task.team_id ? task.account_id === actor : ['owner', 'admin', 'member'].includes(task.role));
...
export const commentSQL = (table = 'tasks', actor = '?1') => `(${table}.archived_at IS NULL AND ((${table}.team_id IS NULL AND ${table}.account_id = ${actor}) OR ${roleSQL(`${table}.team_id`, actor)} IN ('owner','admin','member')))`;
