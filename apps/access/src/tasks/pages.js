...
export function teamPage({ account, team, members, invitations, audit, error = null }) {
  ...
  return page(team.name, `<div class="task-heading"><div><p class="eyebrow">TEAM · ${e(team.role)}</p><h1>${e(team.name)}</h1></div><p class="team-links"><a href="/tasks?team=${e(team.id)}">Team tasks</a><a href="/labels?team=${e(team.id)}">Team labels</a></p></div>${note(error)}
    <div class="team-management${manage ? '' : ' members-only'}">
      ...
      ${manage ? `<section class="task-panel team-invites"><h2>Invite a member</h2>${post(`/teams/${team.id}/invite`, account, `<div class="invite-fields"><label>Verified email${input('email', '', 'email', 'required maxlength="254"')}</label><label>Role${select('role', (team.role === 'owner' ? ['viewer', 'member', 'admin'] : ['viewer', 'member']).map(r => [r, r]), 'viewer')}</label></div>${button('Create invitation')}<p class="fine">The recipient signs in with this verified email and opens Teams. Expires in seven days. No email is sent automatically.</p>`)}<h2>Pending invitations</h2><div class="task-scroll" tabindex="0" aria-label="Pending invitations">${invitations.map(i => `<article class="team-member"><div>${e(i.email)}<span class="task-row-meta">${e(i.role)}</span></div>${canManageMember(team.role, i.role) ? post(`/teams/${team.id}/invitations/${e(i.id)}/revoke`, account, button('Revoke', true)) : ''}</article>`).join('') || '<p class="fine">None.</p>'}</div></section>
