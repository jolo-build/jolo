...
const taskHint = task => `<p class="task-chat-hint">Reference in chat: <code>@codex fix #${taskKeyOf(task)}</code><span class="fine">Revision ${task.revision} · Open the correct local project before sending.</span></p>`;
...
export function teamsPage(account, teams, invitations, error = null, draft = null, personalPrefix = null) {
  return page('Teams', `<div class="task-heading"><div><h1>Teams</h1><p class="fine">Team access starts with an invitation and the recipient’s acceptance.</p>${personalPrefix ? `<p class="fine">Your personal task IDs: <code>${e(personalPrefix)}-1</code>, <code>${e(personalPrefix)}-2</code>, …</p>` : ''}</div></div>${note(error)}
...
export function teamPage({ account, team, members, invitations, audit, error = null, mailEnabled = false }) {
  ...
  return page(team.name, `<div class="task-heading"><div><p class="eyebrow">TEAM · ${e(team.role)} · ${e(team.prefix)}</p><h1>${e(team.name)}</h1></div><p class="team-links"><a href="/tasks?team=${e(team.id)}">Team tasks</a><a href="/labels?team=${e(team.id)}">Team labels</a></p></div>${note(error)}
