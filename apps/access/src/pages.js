...
export function appPage(title, body, { kind = '', active = '', publicPage = false } = {}) {
  const links = publicPage ? [['/', 'Sign in', 'signin'], ['/device', 'Connect a device', 'device']] : [['/tasks', 'Tasks', 'tasks'], ['/chats', 'Chats', 'chats'], ['/teams', 'Teams', 'teams'], ['/labels', 'Labels', 'labels'], ['/account', 'Account', 'account']];
  ...
  return appPage('Approve device', `${heading('Approve device', 'Review this connection before continuing.')}
    <form class="device-approval-form" method="post" action="/device/approve">${csrf(account)}<input type="hidden" name="user_code" value="${e(flow.user_code)}">
      <div class="access-grid access-two-column approval-details task-scroll" tabindex="0" aria-label="Device and requested access">
        ...
        <section class="access-panel" aria-labelledby="permissions-title"><div class="access-panel-body"><h2 id="permissions-title">Requested access</h2><dl class="account-details"><div><dt>Account</dt><dd>${e(account.email)}</dd></div><div><dt>Permissions</dt><dd>${e((flow.scope ?? 'account:read').split(' ').map(scope => ({'account:read':'Identity','tasks:read':'Task access','chats:sync':'Chat sync'})[scope]).join(' · '))}</dd></div></dl><p class="fine">This connects your Jolo account. It grants no access to files or agent tools.</p>${flow.scope?.split(' ').includes('chats:sync') ? '<p class="notice">Chat sync: upload conversation text to your Jolo account and restore it on your other devices. Local files and agent credentials are not uploaded.</p>' : ''}${flow.scope?.split(' ').includes('tasks:read') ? '<p class="notice">Task access: this Jolo profile can read tasks you have permission to view and send referenced task descriptions to your selected coding agent.</p>' : ''}</div></section>
      </div>
