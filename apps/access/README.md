...
Sync includes standalone chats and project task conversations. Restored conversations open as standalone chats in a fresh local directory; reconnect the appropriate project before asking an agent to work on repository files. Files, attachments, tool output, native agent session IDs, credentials, and running processes are not transferred. Each conversation supports up to 1 MB of text and 10,000 messages, with up to 1,000 conversations per account. Oversized conversations remain local and show a sync notice.
...
Deploy migration `0009_chat_sync.sql` with the Access service before shipping desktop chat sync.
