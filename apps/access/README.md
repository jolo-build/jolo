...
Open **Tasks** to create personal tasks or tasks in a team workspace. Tasks open in a read-only detail view; **Edit task** opens the editor, and Save or Cancel returns to the view. Each personal account and each team has an independent ticket sequence starting at `JOLO-1`. Team members share their team’s sequence. Ticket links include the workspace, so the same number in another workspace identifies a different task. Retries do not consume numbers, and archived numbers are never reused. Tasks have descriptions, states, priorities, assignees, and labels. Archive removes a task from active lists and chat lookups; restore makes it available again.
...
For task references in desktop or CLI chat, connect with `jolo login --tasks` or desktop Settings → Account → Connect tasks. Choose a task from the desktop’s `#` picker or paste its full Access link in the correct local project. The link preserves the personal account or team scope. Short references such as `@codex #JOLO-123 fix this problem` work when the number identifies exactly one accessible task; ambiguous numbers require a scoped link. API clients can also supply `team=personal` or a team ID when looking up a short key. Explicitly referenced descriptions are sent to the selected agent. Revoking access prevents new lookups but does not remove content already captured in conversation history. The agent does not automatically update the web task's state.

Migration `0007_workspace_task_numbers.sql` renumbers existing tasks in creation order within each personal account or team. Internal row IDs and comment relationships remain unchanged. Old unscoped web URLs redirect to the original task’s new scoped URL. Apply the migration before deploying the updated Worker; it also assigns scoped numbers to inserts from an older Worker during rollout.
